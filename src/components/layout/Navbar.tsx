...
const Navbar = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 w-full z-[100] transition-all duration-500 ${isScrolled ? "bg-white/90 backdrop-blur-md py-0 border-b border-black/5" : "py-0 bg-transparent"}`}>
      <div className={`max-w-[1400px] mx-auto px-10 flex justify-between items-center text-condensed text-xl tracking-tighter transition-all duration-500 ${isScrolled ? "h-16" : "h-24"}`}>
          ...
          <div className="hidden lg:flex items-center h-full">
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  <span className={`absolute top-0 left-0 right-0 h-1 bg-[#ed1c24] transition-all duration-300 ${isActive ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`} />
        ...
        <div className="flex items-center gap-6">
          <Link href="/quote" className="hidden sm:block border border-black px-6 py-2 text-sm font-bold hover:bg-black hover:text-white transition-all">
            GET A QUOTE
          </Link>
          ...
          >
            <div className="flex flex-col gap-8 text-condensed text-4xl">
              {navItems.map((item) => (
                <a 
                  key={item.name} 
                  href={item.type === "anchor" ? `#${item.name.toLowerCase().replace(" ", "-")}` : "#"} 
                  onClick={(e) => {
                    e.preventDefault();
                    handleNavClick(item);
                  }} 
